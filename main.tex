...
\begin{abstract}
	...
	limited when facing eye motor impairments that affect gaze redirection and fixation.
	This study investigates the feasibility of a gaze-independent visual oddball
	\ac{bci} for individuals with severe eye motor impairment for use with \ac{aac}.
	\emph{Methods:}
	Seven participants with varying degrees of eye motor impairments were
	recruited and visual oddball \ac{bci} decoding accuracy was evaluated under three conditions:
...
vision), ophthalmoplegia (paralysis), fatigability and head motion
limitations can significantly hamper an individual's ability to accurately control
eye gaze, reducing their performance with \acp{bci} that rely on visual
...
Hence, some \ac{bci} and \ac{aac} solutions rely on the capability of the user to direct their
gaze at a desired target.
In this case, the gaze fixation and the \ac{vsa} of the user coincide.
Alternative solutions usually circumvented by designing gaze-independent
\acp{bci}~\cite{Riccio2012}, which either avoid visual stimulation or exploit some form of
...
While these studies were performed with healthy participants,
their ultimate aim was to discover obstacles preventing accurate,
gaze-independent decoding for individuals with gaze impairment.
To that end, they emulate covert \ac{vsa} using central gaze fixation.
This implicitly assumes that \ac{bci} users with eye motor impairment who would benefit
from gaze-independence would feel comfortable performing this central fixation.
...
It does not account for the residual eye motor capabilities of most individuals
with \ac{sspgi}, the (dis)comfort they experience while performing gaze
fixation, and other confounding factors resulting from their eye motility.

It is notable that studies reporting on
gaze-independent visual \ac{bci} use by individuals with \ac{sspgi} are scarce.
...
be used effectively by individuals with \ac{lis}.
...
participants at a later stage of \ac{als} with severe visual impairment.
...
using the visual Hex-o-Spell interface.
...
for a study and they may experience challenges in performing the experiments~\cite{Wolpaw2006}.
...
solutions tailored to their specific situation and residual capabilities.
...
\begin{enumerate*}[label={(\arabic*)}]
	\item the recruitment of individuals that are specifically affected with \ac{sspgi} in
	      a visual \ac{bci} study,
	\item the exploration of their eye motility and experienced comfort when operating such a \ac{bci},
	\item the evaluation of the performance of a gaze-independent visual \ac{bci} for this
	      group, and
	\item the verification of gaze-independent decoding strategy proposed by \textcite{VanDenKerchove2024}.
\end{enumerate*}
Finally, we formulate a set of recommendations based on our experience when carrying out this study, which
could be used in designing further similar studies or \ac{bci}-\ac{aac} solutions
for this target user population.
...
\begin{enumerate*}[label={(\arabic*)}]
	\item complete \ac{lis} without ``useful neuromuscular control,'' including eye movements,
	\item the ability to control a single-button switch using very limited neuromuscular
	      control consisting of eye movements or limited limb movements,
	\item impaired but substantial neuromuscular control including speech or hand control, able to operate
	      a range of classical \ac{aac} devices.
\end{enumerate*}
...
\begin{enumerate}
	\item had any diagnosis of a major medical condition other than neuromuscular
	...
	\item were unable to understand the experiment instructions or to cooperate,
...
During the assessment of visual skills and during the rest of the experimentation
...
\begin{table*}
	...
	\caption[Visual skills of the included participants.]{%
		...
		0.05-0.5: moderate visualimpairment, 0.5-1.3: severe visual impairment, > 1.3 blindness~\cite{WHO2019}).
	}%
...
Similar to this study, the task consists of counting the intensification of a cued
target among 6 circular targets laid out in a hexagonal pattern in the
...
to try this to the maximum extent of their visual skill.
...
neurological conditions~\cite{FriedOken2020}, the number of
blocks was reduced to 6 per \ac{vsa} setting.
In order to decrease task difficulty, \ac{isi} was increased to 200 ms with added random jitter uniformly distributed
between -50 ms and 50 ms.
The experiment also started with a training block with instructions for each
condition, where the participant received oral feedback on their counting
...
accuracy.
Target selection accuracies were obtained for the single-trial case,
as well as aveeraging over 5 and 10 repetitions of all stimuli in a block by
using 6-fold cross-validation with folds corresponding to stimulation blocks.
...
assess the relevance of these settings when gaze is not cued (free \ac{vsa})
...
Target selection accuracy for this task was around chance level ($\frac{1}{6}$) for
...
relying maximally on their residual gaze control.
...
accuracy, likely due to an increased task load, accuracy in free \ac{vsa} where gaze position is uncued can
...
Decoder training with cued, overt \ac{vsa} can improve accuracy in subsequent free \ac{vsa} conditions,
...
Our work shows that some AAC users might benefit from visual \acp{bci}
...
\ac{sspgi} and their specific symptoms and residual skills.
...
real-time, user-centered assessments of comfort and usability to actively develop usable
solutions.
...
%\todo{sub PC4 is self-reported, official oculomotor diagnosis needed}
